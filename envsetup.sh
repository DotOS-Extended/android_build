...
EOF
    T=$(gettop)
    for i in `cat $T/build/envsetup.sh $T/vendor/dot/build/envsetup.sh | sed -n "/^[[:blank:]]*function /s/function \([a-z_]*\).*/\1/p" | sort | uniq`; do
      echo "$i"
    done | column
}
...
function build_build_var_cache()
{
    ...
    cached_vars=`cat $T/build/envsetup.sh $T/vendor/dot/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/get_build_var/) print $(i+1)}' | sort -u | tr '\n' ' '`
    cached_abs_vars=`cat $T/build/envsetup.sh $T/vendor/dot/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/get_abs_build_var/) print $(i+1)}' | sort -u | tr '\n' ' '`
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/dot/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
...
. $ANDROID_BUILD_TOP/vendor/dot/build/envsetup.sh
